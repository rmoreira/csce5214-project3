# -*- coding: utf-8 -*-
"""
Created on Fri Oct 30 08:13:34 2020
...
class FlappyBirdEnv():
    ...
    def reset(self):
        ...
        self.add_pipe = False
        self.episode_step = 0
        self.observation = (round(self.bird.x / WIN_WIDTH * 100) - round((self.pipes[0].x - 232) / WIN_WIDTH * 100)) 
    ...
    def step(self, action):
        ...
        if self.RETURN_IMAGES:
            new_observation = (round(self.bird.x / WIN_WIDTH * 100) - round((self.pipes[0].x - 232) / WIN_WIDTH * 100)) 
...
if start_q_table is None:
    q_table = {}
    for x_diff in range(-500, 500):
            for y_diff in range(2):
                   q_table[(x_diff,y_diff)] = np.random.uniform(-2, 0)
else:
...
for episode in range(NUM_EPISODES):
    ...
    for i in range(10000):
        if np.random.random() > epsilon:
            action = np.argmax(q_table[env.observation, action])
            print(env.observation)
        else:
            action = np.random.randint(0, 2)

        new_obs, reward, done = env.step(action)
        max_future_q = np.max(q_table[new_obs, action])
