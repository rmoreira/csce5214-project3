...
def mainGame(bot=None, episodes=10000, discount=0.9, lr=0.3, SCORES=[]):
    vel = 8
    bird = Bird(200, 200)
    base = Base(730, vel)
    pipes = [Pipe(700, vel)]
    ...
    while run:
        ...
        if len(pipes) > 1:
            if bot.act(bird.x, bird.y, bird.vel, pipes[-1]) == 1:
                bird.jump()
        elif bot.act(bird.x, bird.y, bird.vel, pipes[0]) == 1:
            bird.jump()
        ...
        for pipe in pipes:
            if pipe.collide(bird):
                # print(score)
                bot.update_scores()
                updateQtable(score, bot, episodes, lr, discount, SCORES)
                return True
...
def updateQtable(score, bot, episodes, lr, discount, SCORES):
    ...
    if bot.gameCNT >= episodes:
        ...
        showPerformance(lr, discount, SCORES)
        pygame.quit()
        sys.exit()
...
def showPerformance(lr, discount, SCORES):
    average = []
    num = 0
    sum_s = 0
    ...
    plt.savefig("qtable-LR" + str(lr) + "-D" + str(discount) + ".pdf", format="pdf")
    #plt.show()
...
for i in range(1):
    CLOCK_TICK = 30000
    EPISODES = 2500
    DISCOUNT = round(np.random.uniform(0,1),2)
    print("DISCOUNT RATE= %f" % DISCOUNT)
    LEARNING_RATE = round(np.random.uniform(0,1),2)
    print("LEARNING_RATE = %f" % LEARNING_RATE)
    SCORES = []
    bot = Bot(episodes=EPISODES, discount=DISCOUNT, lr=LEARNING_RATE)
    KeepGoing = True
    while KeepGoing:
        KeepGoing = mainGame(bot=bot, episodes=EPISODES, discount=DISCOUNT, lr=LEARNING_RATE, SCORES=SCORES)
    print("-"*30)
    plt.clf()
