...
env = FlappyBirdEnv()
for k in range(1):
    LEARNING_RATE = 0.29 #round(np.random.uniform(0,1),2)
    ...
    try:
        ...
        for episode in range(EPISODES):
            ...
            while not done:
                ...
                if not done:
                    ...
                    #     print("q_table[discrete_state + (0,)] = %d" % q_table[discrete_state + (0,)])
                    #     print("q_table[discrete_state + (1,)] = %d" % q_table[discrete_state + (1,)])
                    #     print("new_q = %d" % new_q)
        ...
        plt.xlabel("Episode #")
        plt.savefig("qtable-LR"+str(LEARNING_RATE)+"-D"+str(DISCOUNT)+".pdf",format="pdf",bbox_inches='tight')
        #plt.show()
        plt.clf()
        ...
        raise
    #
    with open(f"qtable-LR{str(LEARNING_RATE)}-D{str(DISCOUNT)}.pickle", "wb") as f:
        pickle.dump(q_table, f)

    # with open(f"qtable-latest.pickle", "wb") as f:
    #     pickle.dump(q_table, f)
